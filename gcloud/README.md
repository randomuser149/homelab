# Google Cloud  
This directory contains all files & commands that are and were used on the current Google Cloud VM instance.  
It only runs Caddy and Vaultwarden as of currently.  
Both files are located in the /docker-compose directory on the instance.
Commands used can be found in the [commands.sh](./commands.sh)

## The file structure on the instance (and in this dir as well):
```bash
/  
└── docker-compose/  
    ├── compose.yml  
